...
class Application:
    ...
    def predict(self,test_image):
        ...
        if(self.current_symbol == 'D' or self.current_symbol == 'R' or self.current_symbol == 'U'):
            diction = {}
            diction['D'] = result_dru[0][0]
            diction['R'] = result_dru[0][1]
            diction['U'] = result_dru[0][2]
            diction = sorted(prediction.items(), key=operator.itemgetter(1), reverse=True)
            self.current_symbol = prediction[0][0]

        if(self.current_symbol == 'D' or self.current_symbol == 'I' or self.current_symbol == 'K' or self.current_symbol == 'T'):
            diction = {}
            diction['D'] = result_tkdi[0][0]
            diction['I'] = result_tkdi[0][1]
            diction['K'] = result_tkdi[0][2]
            diction['T'] = result_tkdi[0][3]
            diction = sorted(prediction.items(), key=operator.itemgetter(1), reverse=True)
            self.current_symbol = prediction[0][0]
